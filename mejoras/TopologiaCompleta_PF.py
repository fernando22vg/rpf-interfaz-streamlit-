...
def _resolve_terminal(obj, attr):
    ...
    Cubre dos formas frecuentes:
    - obj.bus1 -> cubicle -> cterm
    - obj.bus1:cterm -> terminal directo
    """
    if obj is None:
        return None
...
def _collect_pf_objects(app, pattern):
    ...
    Orden de búsqueda:
      1. Árbol completo del proyecto activo
      2. Carpeta netdat
      3. GetCalcRelevantObjects (estudio/caso activo)

    Esto evita el caso en el que el estudio activo no tenga cargados todos los
    elementos y GetCalcRelevantObjects devuelva 0, aunque la red sí exista en el
    proyecto.
    """
    candidatos = []
    ...
    # 3) Elementos calculables del estudio/caso activo
    try:
...
# =============================================================================
def export_graph_json(df_buses, df_elements, df_relations, filepath):
    """
    Genera un JSON con nodos (barras) y aristas (elementos de conexion) listo para
    cargar en Streamlit con NetworkX o Plotly Graph Objects.

    Estructura:
      {
        "nodes": [ { "id", "label", "kv", "zone", "v_pu", "v_kv", "angle_deg",
    ...
    for _, row in df_buses.iterrows():
        nodes.append({
            "id": str(row.get("Nombre", "") or ""),
            "label": str(row.get("Nombre", "") or ""),
    ...
    edges = []
    bus_bus = df_relations[df_relations["TipoRelacion"] == "BUS_BUS"] if not df_relations.empty else pd.DataFrame()
    elem_map = {}
    if not df_elements.empty:
        for _, er in df_elements.iterrows():
            elem_map[str(er.get("Nombre", "") or "")] = er
    ...
    for _, row in bus_bus.iterrows():
        ...
        def _eg(k, default=None):
            if isinstance(er, dict):
                return er.get(k, default)
            v = er.get(k) if hasattr(er, "get") else None
        ...
        edges.append({
            ...
            "en_servicio": str(row.get("En servicio", "") or ""),
            "loading_pct": _safe_float(_eg("Carga (%)")),
            "P_from_MW": _safe_float(_eg("P from (MW)") or _eg("P lado 1 (MW)")),
            "Q_from_Mvar": _safe_float(_eg("Q from (Mvar)") or _eg("Q lado 1 (Mvar)")),
            "P_to_MW": _safe_float(_eg("P to (MW)") or _eg("P lado 2 (MW)")),
            "Q_to_Mvar": _safe_float(_eg("Q to (Mvar)") or _eg("Q lado 2 (Mvar)")),
            "perdidas_MW": _safe_float(_eg("Perdidas P (MW)")),
            "kv_nom": _safe_float(_eg("Tension nom. (kV)") or _eg("Tension 1 nom. (kV)")),
            "mva_nom": _safe_float(_eg("Potencia nom. (MVA)")),
            "km": _safe_float(_eg("Longitud (km)")),
            "uktr_pct": _safe_float(_eg("uktr (%)")),
            "curmg_pct": _safe_float(_eg("curmg (%)")),
            "pfe_kW": _safe_float(_eg("pfe (kW)")),
            "pcutr_kW": _safe_float(_eg("pcutr (kW)")),
        })
...
def main():
    ...
    print(f"\n[6/6] Exportando grafo JSON para visualizacion interactiva...")
    export_graph_json(df_buses, df_elements, df_relations, OUTPUT_GRAPH_JSON)
