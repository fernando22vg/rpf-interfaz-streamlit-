...
_script_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "CondInicialesPF.py")
...
if "LOC_GEN_PATH" in _p:
    _code = _patch(_code, r'LOC_GEN_PATH\s*=.*', f'LOC_GEN_PATH = r"{_p["LOC_GEN_PATH"]}"')
if "LOC_CAR_PATH" in _p:
    _code = _patch(_code, r'LOC_CAR_PATH\s*=.*', f'LOC_CAR_PATH = r"{_p["LOC_CAR_PATH"]}"')
if "LOC_XFO_PATH" in _p:
    _code = _patch(_code, r'LOC_XFO_PATH\s*=.*', f'LOC_XFO_PATH = r"{_p["LOC_XFO_PATH"]}"')
