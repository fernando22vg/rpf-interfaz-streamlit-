...
def _ajustar_plini_por_distribuidor(df_plini, dict_plini_dist):
    ...
    for dist, grupo in df_plini.groupby("Distribuidor", sort=False):
        ...
        pnoms = [float(grupo.loc[ix, "P_nom_MW"]) for ix in idxs]
        nonzero_pnoms = [p for p in pnoms if p > 0]
        # Cargas con P_nom=0 reciben como peso el promedio de las no-cero del mismo distribuidor
        fallback_pnom = sum(nonzero_pnoms) / len(nonzero_pnoms) if nonzero_pnoms else 1.0
        effective_pnoms = [p if p > 0 else fallback_pnom for p in pnoms]
        effective_total = sum(effective_pnoms)
        exactas = [objetivo * (ep / effective_total) for ep in effective_pnoms]
...
if not df_plini.empty:
    exact_vals = {}
    for dist, grupo in df_plini.groupby("Distribuidor", sort=False):
        objetivo = dict_plini_dist.get(dist, 0.0)
        pnoms = grupo["P_nom_MW"].values.tolist()
        nonzero_pnoms = [p for p in pnoms if p > 0]
        fallback_pnom = sum(nonzero_pnoms) / len(nonzero_pnoms) if nonzero_pnoms else 1.0
        effective_pnoms = [p if p > 0 else fallback_pnom for p in pnoms]
        effective_total = sum(effective_pnoms)
        for i, idx in enumerate(grupo.index):
            exact_vals[idx] = (objetivo * effective_pnoms[i] / effective_total) if effective_total > 0 else 0.0
    df_plini["plini_exact_MW"] = pd.Series(exact_vals)
else:
